...
fillNavigator();
console.log(Object.keys(navigator.positions).length)
/*fillNavigator();
console.log(Object.keys(navigator.positions).length)
fillNavigator();
console.log(Object.keys(navigator.positions).length)
...
//console.log(Object.keys(navigator.cache).length)
console.log(Object.keys(navigator.positions).length)
const used = process.memoryUsage().heapUsed / 1024 / 1024 / 1024;
console.log(`The script uses approximately ${Math.round(used * 100) / 100} GB`);


const fs = require('fs'); 
const toWrite = JSON.stringify(navigator);
fs.writeFile('navigatorFile.txt', toWrite, function (err) {
  if (err) throw err;
  console.log('Saved!');
});
...
var config = {  
    server: 'rubiks.database.windows.net',  //update me
    authentication: {
        type: 'default',
        options: {
            userName: 'rubiks', //update me
            password: 'Ernopa$$'  //update me
        }
    },
    options: {
        // If you are on Microsoft Azure, you need encryption:
        encrypt: true,
        database: 'solver2x2'  //update me
    }
};  
...
function selectTable() {  
  request = new Request("SELECT  * from solutions", function(err) {  
  if (err) {  
      console.log(err);}  
  });  
  var result = "";  
...
let positionToStore = 'position2';
let solutionToSore = 'solution2';

function insertRow() {  
  request = new Request("INSERT INTO solutions (position, solution) VALUES ('" + positionToStore + "', '" + solutionToStore + "');", function(err) {  
  if (err) {  
      console.log(err);}  
  });  
  var result = "";  
